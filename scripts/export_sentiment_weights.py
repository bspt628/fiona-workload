#!/usr/bin/env python3
"""
export_sentiment_weights.py - Export TinySentiment weights to FIONA C headers

This script converts PyTorch model weights to C header files that can be
compiled directly into FIONA-workload applications.

Usage:
    python export_sentiment_weights.py --model_path ./tiny_sentiment/model.pt --output_dir ../app/sentiment_classifier/weights
...
def write_weight_header(filepath: Path, name: str, arr: np.ndarray, description: str = ""):
    ...
    with open(filepath, "w") as f:
        ...
        f.write(f" * @note Auto-generated by export_sentiment_weights.py\n")
        f.write(f" *\n")
...
def export_model(model_path: str, output_dir: str, config_override: dict = None):
    """Export PyTorch model to FIONA C headers."""
    print("=" * 60)
    print("FIONA Weight Exporter")
    print("=" * 60)
    ...
    # 5. Generate manifest
    print("  - Manifest")
    generate_manifest(state_dict, output_path, config)
...
def export_layer_weights(state_dict: dict, output_path: Path, layer_idx: int, config: dict):
    ...
    # PyTorch TransformerEncoderLayer uses in_proj_weight for Q, K, V combined
    ...
    # LayerNorm weights (Pre-LN: norm1 before attention, norm2 before FFN)
...
def generate_master_header(output_path: Path, config: dict):
    ...
    with open(output_path / "weights.h", "w") as f:
        ...
        f.write(f" * @brief Master header for all TinySentiment weights\n")
        f.write(f" * @note Auto-generated by export_sentiment_weights.py\n")
        f.write(f" */\n\n")
...
def generate_manifest(state_dict: dict, output_path: Path, config: dict):
    ...
    with open(output_path / "manifest.txt", "w") as f:
        f.write("# TinySentiment Weight Manifest\n")
        f.write("# Auto-generated by export_sentiment_weights.py\n")
        f.write("#\n")
...
def main():
    parser = argparse.ArgumentParser(description="Export TinySentiment weights to FIONA C headers")

    parser.add_argument("--model_path", type=str, required=True,
                        help="Path to model.pt checkpoint")
    parser.add_argument("--output_dir", type=str, default="../app/sentiment_classifier/weights",
